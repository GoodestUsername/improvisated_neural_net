...
def normalize_layer(layer):
    ...
    if std_dev == 0:
        # if std dev is zero, then all the values in the list are the same
    ...
    for neuron in layer:
        if neuron.temp < 0:
            # if the value is negative, normalize it using the absolute value of the mean and std dev
            neuron.temp = -((abs(neuron.temp) - mean) / std_dev)
        elif neuron.temp == 0:
            # if the value is zero, set it to the mean value
            neuron.temp = mean
        else:
            # if the value is positive, normalize it using the mean and std dev
...
def normalize_layer_weights(layer):
    for neuron in layer:
        ...
        # calculate mean and std dev for this neuron
        mean = np.mean(parent_values)
        std_dev = np.std(parent_values)

        # check if the std dev is zero for this neuron
        if std_dev == 0:
            # if std dev is zero, then all the values in the list are the same
            # and we cannot normalize the values
            continue
